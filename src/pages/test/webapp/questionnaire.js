  ...
  methods: {
    ...
    updateDisabledNext: function () {
      ...
      if (item.type === 'radio') {
        ...
        if (item.picked.length === 0 || (item.picked.length > 0 && item.picked[0].length < 2))
          flag = true;
      } else {
        if (item.text.length === 0 || item.text[0].length < 10) {
          flag = true;
        }
      }
    ...
    updateDisabledSubmit: function () {
      ...
      this.questions.forEach(function (item) {
        if (item.type === 'radio') {
          ...
          if (item.text.length === 0 || (item.text.length > 0 && item.text[0].length < 10))
